 ...
 * Design: Header com stepper horizontal, pergunta principal,
 * dica em balão, textarea, preview lateral, botões Voltar/Próxima etapa
 */
...
export default function Builder() {
  ...
  return (
    <PhoneFrame>
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        >
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <h1 className="text-white font-bold text-lg leading-tight" style={{ fontFamily: "Nunito, sans-serif" }}>
                Meu Primeiro App
              </h1>
        ...
        >
          <h2 className="text-white text-xl font-bold mb-3" style={{ fontFamily: "Nunito, sans-serif" }}>
        ...
        >
          <div className="relative">
            <textarea
              ...
              className="w-full h-32 bg-transparent border border-white/15 rounded-2xl p-4 text-white text-sm resize-none focus:outline-none focus:border-[#F97316]/50"
              style={{ fontFamily: "Inter, sans-serif" }}
            />
            <span className="absolute bottom-3 right-4 text-white/30 text-xs" style={{ fontFamily: "Inter, sans-serif" }}>
              {text.length}/200
            </span>
          </div>
        </motion.div>
                ...
                <div className="mt-2 space-y-1.5">
                  {["Nome do App", "Nome do Aluno", "Público-alvo", "Objetivo"].map((label, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full ${i === 3 ? "bg-[#F97316]" : "bg-white/20"}`} />
                      <span className={`text-xs ${i === 3 ? "text-[#F97316] font-semibold" : "text-white/40"}`} style={{ fontFamily: "Inter, sans-serif" }}>
                        {label}
                      </span>
                    </div>
                  ))}
      ...
      {/* Bottom buttons */}
      <div className="px-5 py-4 flex gap-3 flex-shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <button
          onClick={() => {
            if (currentStep > 0) {
          ...
          className="flex-1 py-3 rounded-full border border-white/20 text-white font-bold text-sm hover:bg-white/5 transition-colors"
