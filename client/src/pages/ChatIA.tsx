  ...
  return (
    <PhoneFrame>
      ...
      >
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-[#10B981] rounded-full" style={{ border: `2px solid ${t.bg}` }} />
          </div>
          <div>
            <p className="font-bold text-sm" style={{ fontFamily: "Nunito, sans-serif", color: t.textPrimary }}>Tutor IA</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4" style={{ scrollbarWidth: "none" }}>
        {/* Tutor message 1 */}
        <motion.div className="flex gap-2 max-w-[85%]" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="rounded-2xl rounded-tl-md p-4"
            style={{
              background: t.isDark
        ...
        {/* Student message */}
        <motion.div className="flex justify-end" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <div className="rounded-2xl rounded-tr-md p-4 max-w-[85%]"
        ...
        <motion.div className="flex gap-2 max-w-[85%]" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
          <div className="rounded-2xl rounded-tl-md p-4"
            style={{
              background: t.isDark
                ? "linear-gradient(135deg, rgba(124,58,237,0.2) 0%, rgba(124,58,237,0.1) 100%)"
            ...
            <p className="text-sm leading-relaxed mb-3" style={{ fontFamily: "Inter, sans-serif", color: t.textPrimary }}>
              Excelente prompt! Você usou contexto, público-alvo e pediu um formato específico.
            </p>
            <p className="font-bold text-base mb-2" style={{ fontFamily: "Nunito, sans-serif", color: t.textPrimary }}>
              Nota: 9/10!
            </p>
            <div className="flex items-center gap-1">
              {[1, 2, 3, 4].map((s) => (
        ...
        <motion.button
          onClick={() => { toast.success("Missão entregue com sucesso!"); setTimeout(() => setLocation("/home"), 1500); }}
      ...
      {/* Input area */}
      <div className="px-4 py-3 flex items-center gap-2 flex-shrink-0" style={{ borderTop: `1px solid ${t.divider}` }}>
        <div className="flex-1 relative">
          <input
            ...
            className="w-full rounded-full px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-[#F97316]/50"
            style={{
        ...
        <button
          onClick={() => { if (inputText.trim()) { toast("Mensagem enviada (protótipo)"); setInputText(""); } }}
