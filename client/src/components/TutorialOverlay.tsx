    ...
    <TutorialContext.Provider value={{ startTutorial, isTutorialActive: !!activeScreen, completedTutorials, resetTutorials }}>
      ...
      <AnimatePresence>
        {activeScreen && steps.length > 0 && (
          <TutorialOverlayUI steps={steps} currentStep={currentStep} screenId={activeScreen}
            onNext={nextStep} onPrev={prevStep} onClose={closeTutorial} />
        )}
      </AnimatePresence>
...
function TutorialOverlayUI({ steps, currentStep, screenId, onNext, onPrev, onClose }: {
  steps: TutorialStep[]; currentStep: number; screenId: string; onNext: () => void; onPrev: () => void; onClose: () => void;
}) {
    ...
    <motion.div className="fixed inset-0 z-[9999] flex items-center justify-center"
      ...
      {step.spotlightY !== undefined && (
        <motion.div className="absolute left-1/2 -translate-x-1/2 pointer-events-none"
          style={{ width: 320, height: 80, top: `${step.spotlightY}%`, background: `radial-gradient(ellipse, ${color}15 0%, transparent 70%)`, border: `1px solid ${color}20`, borderRadius: "1.5rem" }}
          initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
          transition={{ duration: 0.4 }} key={currentStep} />
      )}

      <motion.div className="absolute left-1/2 -translate-x-1/2 w-[340px] max-w-[90vw]"
        style={{
        ...
        transition={{ duration: 0.4, type: "spring", damping: 20 }} key={currentStep}>
        <div className="rounded-2xl overflow-hidden"
          style={{ background: tooltipBg, border: `1px solid ${color}30`, boxShadow: `0 20px 60px rgba(0,0,0,0.5), 0 0 40px ${color}15` }}>

          <div className="px-5 pt-4 pb-3 flex items-center justify-between" style={{ borderBottom: `1px solid ${t.divider}` }}>
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${color}20` }}>
                {currentStep === 0 ? <Lightbulb size={16} style={{ color }} /> : isLast ? <GraduationCap size={16} style={{ color }} /> : <Sparkles size={16} style={{ color }} />}
              </div>
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ fontFamily: "Inter, sans-serif", color: t.textMuted }}>
                  Tutorial — {screenId}
                </p>
                <div className="flex items-center gap-1 mt-0.5">
                  {steps.map((_, i) => (
                    <div key={i} className="h-1 rounded-full transition-all duration-300"
                      style={{ width: i === currentStep ? 16 : 6, background: i <= currentStep ? color : t.divider }} />
                  ))}
                </div>
              </div>
            </div>
            <button onClick={onClose} className="w-7 h-7 rounded-full flex items-center justify-center transition-colors" style={{ background: t.cardBg }}>
          ...
          <div className="px-5 py-4">
            {step.emoji && (
              <motion.div className="text-3xl mb-3" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", damping: 10, delay: 0.15 }}>
                {step.emoji}
              </motion.div>
            )}
            <motion.h3 className="font-bold text-lg leading-tight mb-2" style={{ fontFamily: "Nunito, sans-serif", color: t.textPrimary }}
              initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
              {step.title}
            </motion.h3>
            <motion.p className="text-sm leading-relaxed" style={{ fontFamily: "Inter, sans-serif", color: t.textSecondary }}
          ...
          <div className="px-5 pb-4 pt-2 flex items-center justify-between">
            <div className="flex items-center gap-2">
              {!isFirst && (
                <button onClick={onPrev} className="flex items-center gap-1 px-3 py-2 rounded-xl transition-colors text-xs font-semibold"
                  style={{ fontFamily: "Inter, sans-serif", background: t.cardBg, color: t.textSecondary }}>
                  <ChevronLeft size={12} />Anterior
                </button>
              )}
              <button onClick={onClose} className="text-xs transition-colors px-2 py-2"
            ...
            <motion.button onClick={onNext}
              ...
              style={{ fontFamily: "Nunito, sans-serif", background: `linear-gradient(135deg, ${color}, ${color}cc)`, boxShadow: `0 4px 15px ${color}40` }}
              whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
              {isLast ? (<>Concluir<GraduationCap size={14} /></>) : (<>Próximo<ChevronRight size={14} /></>)}
            </motion.button>
          </div>

          <div className="px-5 pb-3 text-center">
            <span className="text-[10px] font-medium" style={{ fontFamily: "Inter, sans-serif", color: t.textMuted }}>
